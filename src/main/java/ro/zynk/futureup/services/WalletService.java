...
public class WalletService {
    ...
    public CoinTransactionResponse buyCoin(CoinTransactionRequest buyCoinRequest) throws NotFoundException {
        ...
        double totalValue = coin.getValue() * coinAmount.getAmount();
        Transaction transaction = new Transaction(coin, coinAmount.getAmount(), totalValue);
    ...
    public ValueResponse getAllCoinsFromWalletInUSA (Long walletId) throws NotFoundException {
        Optional<Wallet> walletOpt = walletRepository.findById(walletId);
        if (walletOpt.isEmpty()) {
            throw new NotFoundException("Wallet not found!");
        }
        ...
        for (CoinAmount coinAmount : coinAmounts) {
            amount = amount + coinAmount.getAmount().floatValue() * coinAmount.getCoin().getValue().floatValue();
        }
        return new ValueResponse(amount);
    }
}
