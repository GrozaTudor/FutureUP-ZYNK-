...
public class WalletController {
    ...
    @GetMapping (value = "/get_all_owned_coins_in_USA")
    public ResponseEntity<BaseResponse> getAllOwnedCoinsFromWalletInUSA (@RequestParam("walletId") Long walletId) {
        try {
            return new ResponseEntity<>(walletService.getAllCoinsFromWalletInUSA(walletId), HttpStatus.OK);
        } catch (NotFoundException e) {
            return new ResponseEntity<>(new ErrorResponse(e.getMessage()), HttpStatus.BAD_REQUEST);
        }
    }
